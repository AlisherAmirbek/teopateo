...
- Insights can apply a plan suggestion directly by creating a missing trigger rule.

## App Impact

- `PlanView` now manages trigger rules, personal reasons, replacement activities, taper settings, and reminders from one plan surface.
...
- SQLite schema version 5 adds taper check-in fields and legacy risky-situation records.

## Tests

- Persistence tests cover taper target check-in fields and legacy risky-situation records.
- Store tests cover trigger rule, reason, replacement activity, taper schedule, check-in target, and insight-apply behavior.
